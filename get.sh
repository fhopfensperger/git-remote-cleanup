...
: ${BINARY_NAME:="git-remote-cleanup"}
: ${USE_SUDO:="true"}
...
verifySupported() {
  ...
  if ! echo "${supported}" | grep -q "${OS}-${ARCH}"; then
    ...
    echo "To build from source, go to https://github.com/fhopfensperger/git-remote-cleanup"
    exit 1
  fi
...
checkDesiredVersion() {
  if [ "x$DESIRED_VERSION" == "x" ]; then
    ...
    TAG=$(curl --silent "https://github.com/fhopfensperger/git-remote-cleanup/releases/latest" | sed 's#.*tag/\(.*\)\".*#\1#')
  else
    TAG=$DESIRED_VERSION
  fi
}

# checkInstalledVersion checks which version of git-remote-cleanup is installed and
...
downloadAndInstall() {
  ...
  downloadUrl=$(curl -s https://api.github.com/repos/fhopfensperger/git-remote-cleanup/releases/latest | grep browser_download_url | grep ${OS}_${ARCH} | cut -d ":" -f 2,3 | cut -d " " -f 2,3 | tr -d \")
